...
import { fetchPostSlugs } from '../utils/contentfulPosts'

const MAIN_URL =
  process.env.siteUrl.slice(-1) === '/'
    ? process.env.siteUrl.substring(0, process.env.siteUrl.length - 1)
    : process.env.siteUrl
...
class Sitemap extends React.Component {
  static async getInitialProps({ res }) {
    const blogPostSlugs = await fetchPostSlugs()
